...
class ProjectAxisDashboard(ctk.CTk):
    ...
    def verify_with_firebase(self, email, password):
        try:
            ...
            # Step B: Get User's Document from Firestore
            ...
            # Extract status and suspendReason from Firestore fields
            status = db_data["fields"].get("status", {}).get("stringValue", "active")
            suspend_reason = db_data["fields"].get("suspendReason", {}).get("stringValue", "No reason provided.")

            if status == "suspended":
                # Block login and show suspension message
                error_msg = (
                    f"⛔ Your account has been suspended!\n\n"
                    f"Reason: {suspend_reason}\n\n"
                    f"Please contact ProjectAxis Admin to reactivate your account. (📞 +91 7695958035)"
                )
                self.show_error(error_msg)
                return  # Exit immediately, do NOT proceed to device lock logic

            # --- Device Lock Logic (only if account is active) ---
            ...
            if saved_device_id == "":
                ...
                update_payload = {
                    "fields": {
                        "deviceID": {"stringValue": self.system_uuid},
                        "createdAt": db_data["fields"].get("createdAt", {})
                    }
                }
